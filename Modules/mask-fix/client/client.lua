	...
	if not DoesShopPedApparelHaveRestrictionTag(maskHash, `SHRINK_HEAD`, 0) then
		if isHeadShrunken then
			CreateThread(function()
				...
				print('mask-fix Saved used')
				print("savedBlendData", json.encode(savedBlendData))
				print('savedFaceFeatures', json.encode(savedFaceFeatures))
			end)
		end
		return
	end
...
CreateThread(function()
	while true do
		while exports['illenium-appearance']:isCustomMenuOpen() do Wait(500) end
