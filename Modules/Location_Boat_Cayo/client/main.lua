...
OnlyLife.Client.Location_Cayo = RageUI.CreateMenu('Location_Cayo', 'Location_Cayo')
OnlyLife.Client.Location_Cayo.Closed = function()
...
function OnlyLife.Client:OpenMenuarivant()
        if OnlyLife.Client.open then
            OnlyLife.Client.open = false
            RageUI.Visible(OnlyLife.Client.Location_Cayo, false)
            return
    else
            OnlyLife.Client.open = true
            RageUI.Visible(OnlyLife.Client.Location_Cayo, true)
            CreateThread(function()
                while OnlyLife.Client.open do
                    FreezeEntityPosition(PlayerPedId(), true)
                    RageUI.IsVisible(OnlyLife.Client.Location_Cayo, function()
                        for k, v in pairs(Location_Cayo_cfg.location) do
                            RageUI.Button(v.label, nil, {RightLabel = '$'..v.price}, true, {
                                onSelected = function()
                                    ESX.TriggerServerCallback('Onlylife:Location_Cayo2.checkMoney', function(payed)
...
function OnlyLife.Client:SpawnVehicle(car)
    ...
    --    Citizen.Wait(0)
    --end
    for k, v in pairs(Location_Cayo_cfg.Spawn) do
        if ESX.Game.IsSpawnPointClear(vector3(v.x, v.y, v.z), 2.0) then
            TriggerServerEvent('Onlylife:Location_Cayo2.spawnBoat', car, vector3(v.x, v.y, v.z), v.h)
            break
        end
    end
end
Citizen.CreateThread(function()
        while true do
            local Waito = false
            for _, info in pairs(Location_Cayo_cfg.Position) do
