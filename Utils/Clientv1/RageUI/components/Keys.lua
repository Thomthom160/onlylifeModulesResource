...
function Keys.Register(Controls, ControlName, Description, Action)
    ...
    RegisterKeyMapping(string.format('OnlylifeUI-%s', ControlName), Description, "keyboard", Controls)
    RegisterCommand(string.format('OnlylifeUI-%s', ControlName), function(source, args)
        if (Action ~= nil) then
            Action();
        end
    end, false)
